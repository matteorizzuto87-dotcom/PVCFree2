// PVC-FREE K2025 — db-patch.js (v5 resilient + a11y)
(function(){
  const VERSION = "v5";
  console.info(`[db-patch.js ${VERSION}] loaded`);

  // ---- Neutralize legacy that throws on DB tab ----
  try { window.loadMaterials = function(){}; } catch(e){}

  // ---- Optional: hook switchTab to ensure our UI builds when DB tab is opened ----
  const originalSwitchTab = window.switchTab;
  window.switchTab = function(view){
    try { if (typeof originalSwitchTab === "function") originalSwitchTab(view); } catch(e){}
    if (/material/i.test(String(view||""))) {
      setTimeout(start, 0);
    }
  };
  ...
  // ---- Find the Database section robustly ----
  function findSection(){
    ...
    // As extreme fallback, pick the first large section inside main after tabs
    const mains = document.querySelectorAll("main, .main, .container");
    for (const m of mains){
      const sec = m.querySelector("section"); if (sec) return sec;
    }
    return null;
  }

  function buildUI(container){
    if (!container) { console.warn("[db-patch v5] container NON trovato"); return false; }
    if (container.__dbUIBuilt) { console.info("[db-patch v5] UI già presente"); return true; }
    ...
    console.info("[db-patch v5] UI costruita");
    return true;
  }
  ...
  function start(){
    ensurePapa(function(){
      ...
      if (!container){ console.warn("[db-patch v5] sezione Database non trovata"); return; }
      if (buildUI(container)) initLogic();
    });
  }

  // Try on load and when user clicks tabs
