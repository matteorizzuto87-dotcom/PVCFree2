// PVC-FREE K2025 — db_patch.js (v4, a11y + cache-bust hints)
(function(){
  window.__dbPatchVersion = "v4";
  console.info("[db-patch.js v4] loaded");

  // Neutralize legacy function that may error on tab switch
  try { window.loadMaterials = function(){}; } catch(e){}

  const CSV_PATH = "./data/materiali_database.csv";

  // Load PapaParse if missing
  ...
  const toNumber = x => {
    ...
    const m = String(x).toLowerCase().replace(",", ".").match(/(\d+(\.\d+)?)/);
  ...
  function buildUI(){
    ...
    if (!section){ console.warn("[db_patch v4] #view-materials non trovato"); return false; }
      ...
      <div class="card card-pad">
        ...
        <div class="grid" style="margin-top:12px">
          <div class="col-4">
            ...
            <select id="f-polimero" name="polimero" aria-labelledby="db-title f-polimero-label">
          ...
          <div class="col-4">
            <label for="f-applicazione" id="f-applicazione-label">Applicazione</label>
            <select id="f-applicazione" name="applicazione" aria-labelledby="db-title f-applicazione-label">
              <option value="">Tutte</option>
  ...
  function start(){
    ensurePapa(function(){
      const ok = buildUI();
      if (ok) initLogic();
      else console.warn("[db_patch v4] UI non costruita (manca #view-materials)");
    });
  }
