// PVC-FREE K2025 — db_patch.js (tailored for "index (2).html")
(function(){
  // 1) neutralizza la funzione legacy per evitare errori quando si apre la tab
  try { window.loadMaterials = function(){}; } catch(e){}

  // 2) path CSV
  const CSV_PATH = "./data/materiali_database.csv";

  // 3) carica PapaParse se manca
  ...
  // 5) crea UI nella sezione #view-materials
  function buildUI(){
    const section = document.querySelector("#view-materials");
    if (!section) return false;
    // sostituisce l'HTML di quella sezione con l'interfaccia richiesta
    section.innerHTML = `
